...
const TOKEN_PLAN_URL: &str = "https://home.qwencloud.com/billing/subscription/token-plan-individual";
const USAGE_URL: &str =
    "https://home.qwencloud.com/data/api.json?action=IntlBroadScopeAspnGateway";
...
pub fn normalize_usage(body: &[u8]) -> Result<Usage, FetchError> {
    let response: GatewayResponse = serde_json::from_slice(body)
        .map_err(|error| FetchError::Decode(format!("qwen-cloud response not decodable: {error}")))?;
    ...
    if response.success_response != Some(true) {
        return Err(degraded_response_error(result, "gateway did not report success"));
    }
...
mod tests {
    ...
    fn normalizes_live_har_token_plan_response() {
        ...
        assert!((primary.used_percent - 13.117_665_718_963_334).abs() < 0.000_001);
        ...
        assert!((secondary.used_percent - 5.383_497_228_290_000_4).abs() < 0.000_001);
    ...
    fn clamps_used_fraction_above_one() {
        ...
        body["data"]["DataV2"]["data"]["data"]["per5HourPercentage"] =
            serde_json::json!(1.5);
